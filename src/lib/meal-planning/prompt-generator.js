/*
This file exists because prompt-generator.ts can only use the db client in another TypeScript file.
...
This file is a JavaScript file because it's used in the API route.
*/
...
export class MealPlanPromptGenerator {
   ...
   */
  static async fetchMealPlanData(staplesOnly = false) {
    ...
    const ingredients = await getRows(ingredientsQuery);
    ...
    const mealCombos = await getRows(mealCombosQuery);
   ...
   */
  static generatePrompt(data, userGoals) {
...
${suggestions}`;
   ...
   */
  static async generateCompletePrompt(userGoals, staplesOnly = false) {
